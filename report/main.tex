...
\newline
Al fine di effettuare le dovute verifiche prima dell'attivazione di una fornitura, gli operatori dovranno avere modo di visualizzare tutti i dettagli delle richieste e di aggiornarne lo stato (approvata o respinta). Una richiesta può essere assegnata a un solo operatore.
\newline
...
\newline
Le bollette sono associate a uno specifico contratto e possono essere di due tipi: calcolate sui consumi effettivi oppure calcolate attraverso stime pro capite. Per ciascuna bolletta vengono memorizzati data di emissione, periodo di riferimento (con data di inizio e di fine), data di scadenza, eventuale data di pagamento, importo da pagare, consumi e un documento contenente informazioni di dettaglio che verrà caricato manualmente dagli operatori.
\newline
...
\newline
Al fine di effettuare le dovute verifiche prima dell'attivazione di una fornitura, gli operatori dovranno avere modo di \textit{visualizzare tutti i dettagli} delle richieste e di \textit{aggiornarne} lo stato (approvata o respinta). Una richiesta può essere \textit{assegnata} a un solo operatore per volta. Un contratto è considerato attivo se la relativa richiesta è stata approvata e se non è presente una richiesta di cessazione approvata relativa a quello stesso contratto.
\newline
...
\newline
Le \textbf{bollette} sono \textit{associate} a uno specifico \textbf{contratto} e possono essere di due tipi: calcolate sui consumi effettivi oppure calcolate attraverso stime pro capite. Per ciascuna bolletta vengono memorizzati data di emissione, periodo di riferimento (con data di inizio e di fine), data di scadenza, eventuale data di pagamento, importo da pagare, consumi e un documento contenente informazioni di dettaglio che verrà caricato manualmente dagli operatori.
\newline
...
In \cref{fig:requests-activation} sono mostrate le varie associazioni che coinvolgono le due tipologie di richieste. Una richiesta di cessazione è sempre associata a un solo contratto, mentre per uno stesso contratto possono essere create più richieste di cessazione.
\newline
...
\begin{longtable}{l p{10cm} c r}
    ...
    \textbf{OP-27} & Visualizzare la media dei consumi (reali e stimati) per un contratto in un dato periodo & 1.000/mese \\
    \textbf{OP-28} & Visualizzare la media aggregata dei consumi (reali e stimati) di una data materia prima prodotti in un dato periodo e relativi a tutti i contratti attivi in un dato comune & 1.000/mese \\
...
\paragraph{OP-29: Visualizzare la media aggregata dei consumi (reali e stimati) relativi a tutti i contratti attivi in un dato comune}\mbox{}\\
...
\begin{lstlisting}
insert into letture (MatricolaContatore, DataEffettuazione, Consumi, Stato, Note, IdPersona)
select :meterId, curdate(), :consumption, "In lavorazione", :notes, :personId
from letture L
where (select L2.Consumi
        from letture L2
        where L2.MatricolaContatore = :meterId
        and L2.DataEffettuazione <= curdate()
        order by L2.Consumi desc
        limit 1) <= :consumption
...
\paragraph{OP-18: Inserire una nuova bolletta per un contratto attivo}\mbox{}\\
\begin{lstlisting}
insert into bollette (DataEmissione, DataInizioPeriodo, DataFinePeriodo, DataScadenza, Importo, Consumi, DocumentoDettagliato, Stimata, IdOperatore, IdContratto)
select curdate(), :intervalStartDate, :intervalEndDate, :deadline, :finalCost, :consumption, :reportFile, :estimated, :operatorId, :subId
...
\paragraph{OP-27: Visualizzare la media dei consumi (reali e stimati) per un contratto in un dato periodo}\mbox{}\\
\begin{lstlisting}
...
\paragraph{OP-28: Visualizzare la media aggregata dei consumi (reali e stimati) di una data materia prima prodotti in un dato periodo e relativi a tutti i contratti attivi in un dato comune}\mbox{}\\
\begin{lstlisting}
...
\begin{lstlisting}
select O1.CodOfferta, O1.Nome, count(C1.IdContratto) as NumeroContratti
from offerte O1, contratti C1
where C1.Offerta = O1.CodOfferta
group by O1.CodOfferta
order by Conteggio desc
limit 1;

\end{lstlisting}

\paragraph{OP-32: Visualizzare il numero di contratti attivati in un dato anno}
\begin{lstlisting}
select count(*)
from `contratti approvati` C
where year(C.DataChiusuraRichiesta) = ?;
...
Per lo sviluppo dell'applicazione si è scelto come DBMS di riferimento \textit{MySQL}. Il front-end, sviluppato in Java, segue il pattern architetturale MVC e utilizza la libreria grafica \textit{JavaFX}. Per la comunicazione col database si è optato per la libreria \textit{jOOQ}, basata sull'API \textit{JDBC} e rispetto alla quale offre un più alto livello di astrazione.
\newline
L'applicazione si presenta all'avvio con una schermata in cui è possibile consultare liberamente il catalogo delle offerte, effettuare l'accesso oppure, se non lo si è già fatto, registrarsi.
\newline
L'inserimento di una richiesta di contratto avviene mediante una serie di passaggi consecutivi in cui l'applicazione verifica a ogni nuova schermata la correttezza dei dati inseriti dall'utente, così da garantire la massima integrità dei dati. Nel caso in cui l'inserimento non vada a buon fine o venga interrotto, nessun dato verrà inserito nel database.
