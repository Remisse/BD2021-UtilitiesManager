...
\begin{longtable}{l p{10cm} c r}
    ...
    \textbf{OP-19} & Assegnare una richiesta a un operatore & 20.007/mese \\
...
values (?, ?, ?);
    ...
    \item \textbf{OP-5: Inserire una richiesta di contratto con attivazione tramite voltura}
    \begin{lstlisting}
...
values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    \item \textbf{OP-6: Selezionare l'eventuale contratto attivo intestato a un dato cliente e associato a un dato contatore (voltura)}
    \begin{lstlisting}
select contratti.*
from contratti, immobili, contatori, offerte
where contratti.IdCliente = ?
and contratti.IdImmobile = immobili.IdImmobile
and immobili.IdImmobile = contatori.IdImmobile
and contatori.Matricola = ?
and contratti.
    \end{lstlisting}

    \item \textbf{OP-7: Inserire una richiesta di contratto con nuova attivazione}
    \begin{lstlisting}
insert into contratti (DataAperturaRichiesta, DataChiusuraRichiesta, StatoRichiesta, NoteRichiesta, Offerta, Uso, TipoAttivazione, NumeroComponenti, IdImmobile, IdCliente)
values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    \end{lstlisting}

    \item \textbf{OP-13: Inserire una nuova offerta}
    \begin{lstlisting}
insert into offerte
values (default, ?, ?, ?, ?, ?);
...
values (last_insert_id(), ?);
    ...
    \item \textbf{OP-14: Aggiornare un'offerta}
    \begin{lstlisting}
update offerte
