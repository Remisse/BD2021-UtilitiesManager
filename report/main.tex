...
\begin{itemize}
    ...
    \item Matricola del contatore del gas: 38256395028277
\end{itemize}
\end{mdframed}
...
\begin{figure}[H]
\centering{}
\makebox[\textwidth][c]{\includegraphics[width=0.65\textwidth]{images/personER.png}}
...
\begin{figure}[H]
\centering{}
\makebox[\textwidth][c]{\includegraphics[width=0.65\textwidth]{images/consumpER.png}}
\caption{Letture.}
...
\begin{lstlisting}
select cod, nome, NumeroContratti
from (select t1.*, rank() over (order by NumeroContratti desc) as rk
      from (select O.CodOfferta as cod, O.Nome as nome, count(*) as NumeroContratti
            from offerte O, contratti C
            where C.Offerta = O.CodOfferta
            and O.MateriaPrima = "Gas"
            group by O.CodOfferta) t1) t
where rk = 1;
\end{lstlisting}
